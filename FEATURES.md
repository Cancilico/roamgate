...
- `Cmd/Ctrl`-click HTTP(S) links to open a browser tab; file/directory paths open
  ...
  (or a one-tap **Open link** button when the lookup outlasts the tap's
  activation) and paths open file actions. A tap that closes the device
  keyboard only closes it. Long-press still selects text, with **Open link** or
  **File actions**. Nothing opens on hover.
